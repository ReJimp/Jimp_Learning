...
- 分页式
  ...
  - 基本思想：物理内存被分成固定长且比较小的存储块（页框、实页、物理页），每个进程也被划分成固定长的程序块（页、虚页、逻辑页），无需用连续页框来存放一个进程（使用页表进行映射），操作系统为每个进程生成一个页表，通过页表（Page Table）实现逻辑地址向物理地址转换（Address Mapping）
...
- 逻辑地址转换为线性地址
  ...
  - 段选择符&段寄存器
    ...
    - 段选择符
      ...
      - TI=0，选择全局描述符表(GDT)，TI=1，选择局部描述符表(LDT)；RPL=00，为第0级，位于最高级的内核态，RPL=11，为第3级，位于最低级的用户态，第0级高于第3级；高13位索引用来确定当前使用的段描述符在描述符表中的位置
...
- 中断描述符表
  ...
  <img src="./img/SegmentDescriptor.svg"  />
